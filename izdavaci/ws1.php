...
function setIzdanje() {
	if (!isset($_POST['razred_id']) || !isset($_POST['jezik_id']) || !isset($_POST['predmet_id']) || !isset($_POST['naziv']) || !isset($_POST['autori']) || !isset($_POST['naziv_udzb_jedinice']) || !isset($_POST['resenje']) || !isset($_POST['godina']) || !isset($_POST['mediji_id'])) {
		...
		require_once ('../includes/db_connection.php');
		$sql = "INSERT INTO izdanja.izdanja(naziv, autori, resenje_ministarstva, naziv_udzb_jedinice, foramti_izdanja_id, broj_strana, jezici_id, razredi_id, izdavac_id, mediji_id, predmet_id, godina_izdanja)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
		$stmt = $pdo->prepare($sql);
...
function getAllIzdanja() {
	...
	WHERE izdanja.izdanja.foramti_izdanja_id = pratece.formati_izdanja.formatizdanja_id
...
function getIzdanje () {
	...
	$sql = "SELECT idanja_id, naziv, autori, resenje_ministarstva, naziv_udzb_jedinice, foramti_izdanja_id, broj_strana, jezici_id, godina_izdanja, razredi_id, izdavac_id, mediji_id, predmet_id
...
function editIzdanje () {
	if (!isset($_POST['izdanja_id']) || !isset($_POST['razred_id']) || !isset($_POST['jezik_id']) || !isset($_POST['predmet_id']) || !isset($_POST['naziv']) || !isset($_POST['autori']) || !isset($_POST['naziv_udzb_jedinice']) || !isset($_POST['resenje']) || !isset($_POST['format_id']) || !isset($_POST['mediji_id']) || !isset($_POST['br_strana']) || !isset($_POST['godina'])) {
		...
		$sql = "UPDATE izdanja.izdanja
				SET naziv = ?, autori = ?, resenje_ministarstva = ?, naziv_udzb_jedinice = ?, foramti_izdanja_id = ?, broj_strana = ?, jezici_id = ?, razredi_id = ?, mediji_id = ?, predmet_id = ?, godina_izdanja = ?
				WHERE idanja_id = ?;";
		$stmt = $pdo->prepare($sql);
...
function editProfilIzdavaca() {
	...
	if (!isset($_POST['naziv']) || !isset($_POST['pib']) || !isset($_POST['maticni_broj']) || !isset($_POST['grad_id']) || !isset($_POST['lozinka']) || !isset($_POST['potvrda_lozinke']) || !isset($_POST['adresa']) || !isset($_POST['email'])) {

		$rtn = array('status' => 0, 'msg' => 'Nisu poslata sva polja !');
		exit(json_encode($rtn));
	}
	else if (empty($_POST['naziv']) || empty($_POST['pib']) || empty($_POST['maticni_broj']) || empty($_POST['grad_id']) || empty($_POST['lozinka']) || !isset($_POST['potvrda_lozinke']) || empty($_POST['adresa']) || empty($_POST['email'])) {
		...
		$sql = 'UPDATE izdavaci.izdavaci
				SET naziv=?, "PIB"=?, maticni_broj=?, grad_id=?, lozinka=?, adresa=?, email=?
				WHERE izdavac_id = ?;';
		$data = [$naziv, $pib, $maticni_broj, $grad_id, sm_encrypt($lozinka), $adresa, $email, $izdavac_id];
